...
model.age.incidence.series.age.alt <- function(par.dat, age_vect, year.start, age_mult){
  ...
  for (i in 1:lts){
    ...
    for(a in 2:(length(age_vect_tmp))){ 
      ...
      age.mult.df <- cbind.data.frame(mult=age_mult, 
                                      age_min=c(1,4,16,21,31,41,51,61,71,81),
                                      age_max=c(3,15,20,30,40,50,60,70,80,90))
      age.mult.df$dur = (age.mult.df$age_max-age.mult.df$age_min)+1
...
log.lik.fit.age.alt <- function(par.dat, age.mult, dat){
  
  
  age_vect=seq(0, max(dat$age), by=1)
  
  
  out.mod <- model.age.incidence.series.age.alt(par.dat= par.dat, year.start=min(par.dat$year), age_mult=age.mult$age_mult, age_vect=age_vect)
...
run.comp.models <- function(dat,par.dat, age.mult1, age.mult2){
  ...
  par.dat$llik_age_1 <- log.lik.fit.age(par.dat = par.dat, age.mult=age.mult1, dat=df.out)
  
  par.dat$llik_age_2 <- log.lik.fit.age.alt(par.dat = par.dat, age.mult=age.mult2, dat=df.out)
  
  par.slim <- dplyr::select(par.dat, llik, llik_age_1, llik_age_2, provname)
  ...
  par.slim$AIC_age1 <- (2*(nrow(par.dat)+10))+(2*par.slim$llik_age_1)
  par.slim$AIC_age2 <- (2*(nrow(par.dat)+10))+(2*par.slim$llik_age_2)
...
age.orig <- out
load("fit-all-provinces-age-mult-alt.Rdata")
age.alt <- out
...
comp.split <- mapply(run.comp.models, dat=dat.split, par.dat=fit.split, MoreArgs = list(age.mult1 = age.orig, age.mult2 = age.alt), SIMPLIFY = F)
