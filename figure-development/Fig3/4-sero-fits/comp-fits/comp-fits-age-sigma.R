...
log.lik.wane.fit.age <- function(par.dat, dat, sigma.vector){
  
  age_vect=seq(0, max(dat$age), by=1)
  
  out.mod <- model.age.incidence.series.wane.age(par.dat= par.dat, year.start=min(par.dat$year), age_vect=age_vect, sigma.df=sigma.vector)
...
model.age.incidence.series.wane.age <- function(par.dat, age_vect, year.start, sigma.df){
  ...
  for (i in 1:lts){
    ...
    for(a in 2:(length(age_vect_tmp))){ 
      ...
      #set up sigma
      sigma.age.df <- cbind.data.frame(wane=sigma.df, 
                                       age_min=c(1,6,11,16,21,31,41,51,61,71),
                                       age_max=c(5,10,15,20,30,40,50,60,70,100))
      
      
      
      sigma.age.df$dur = (sigma.age.df$age_max-sigma.age.df$age_min)+1
      
      #rep it as a multiplier by year
      sigma.age.vect = c(unlist(mapply(rep, x=as.list(sigma.age.df$wane), each=as.list(sigma.age.df$dur), SIMPLIFY = F)))
      
      #now, make it the length of dur for which there are not 0s
      sigma.age.vect = sigma.age.vect[1:length(dur[dur>0])]
      
      sigma = c(dur[dur==0], sigma.age.vect)
      
      if(length(  sigma) != length(dur)){
        warning(paste0("sigma vector incorrect length at age=", age, " and i=", i))
      }
...
run.comp.models <- function(dat,par.dat, sigma.df, age.mult){
  ...
  # sigma.merge <- dplyr::select(sigma.df, year, sigma)
  # 
  # par.dat <- merge(par.dat, sigma.merge, by="year", all.x = T)
  # par.dat$sigma[is.na(par.dat$sigma)] <- 0
  # par.dat$sigma[par.dat$sigma < 0.000010] <- 0
  # #and sigma fit
  par.dat$llik_sigma = log.lik.wane.fit.age(par.dat = par.dat, dat=df.out, sigma.vector = sigma.df$sigma)
  ...
  par.slim$AIC_sigma <- (2*(nrow(par.dat)+nrow(sigma.df)))+(2*par.slim$llik_sigma)
...
load("sigma-fit-all-age-new.Rdata")
...
comp.split <- mapply(run.comp.models, dat=dat.split, par.dat=fit.split, MoreArgs = list(sigma.df = sigma.fit, age.mult = out), SIMPLIFY = F)
