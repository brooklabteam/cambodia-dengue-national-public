...
sigma.fit <- dplyr::select(sigma.fit, year, sigma)
fit.dat <- merge(fit.dat, sigma.fit, by="year", all.x = T)
fit.dat$sigma[is.na(fit.dat$sigma)] <- 0
lts.sigma = nrow(sigma.fit)
...
log.lik.wane.fit.profile <- function(par, par.dat, dat){
...
profile.sigma <- function(yr.index, dat, sigma.test, par.dat){
  
  #replace sigma with the one from the range
  par.dat$sigma[par.dat$year==yr.index] <- sigma.test
...
test.profile <- function(yr.index, dat1, range.sigma, par.dat){
  
  out.llik <- list()
  for(i in 1:length(range.sigma)){
    out.llik[[i]] <- profile.sigma(yr.index=yr.index, dat=dat1, sigma.test=range.sigma[i], par.dat=par.dat)
  }
  
  ll1 <- c(unlist(out.llik))
  
  
  tmp2=smooth.spline(range.sigma,ll1)
  new=seq(0, 0.5, length.out=1000)
  ...
  #plot(range.sigma,ll1, type="b")
  #abline(v=conf.int, col="blue", lty=2)
  
  ci.df = cbind.data.frame(year = yr.index, lci = conf.int[1], uci=conf.int[2])
  return(ci.df)
}
profile.wane.LBFGSB.all <- function(dat,par.dat, sigma.vect){
  ...
  range.sigma = seq(0, 0.5, length.out=500)
  
  #make list of years of sigma to test 
  out.sigma.df <- data.table::rbindlist(lapply(as.list(sigma.vect), test.profile, dat1=df.out, range.sigma=range.sigma, par.dat=par.dat))
...
out.sigma.CI <- profile.wane.LBFGSB.all(dat=dat, par.dat=fit.dat, sigma.vect = c(2002:2020))


save(out.sigma.CI, file = "sigma-profile.Rdata")
